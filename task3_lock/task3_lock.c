...
struct completion empty; // declare completion variable empty which indicates condition variable
spinlock_t lock; // declare spinlock variable lock
...
static int producer_func_odd(void *data) {
	...
	while(!kthread_should_stop()) {
		...
		complete(&empty); // send signal to consumer model
...
static int consumer_func1(void *data) {
	...
	while(!kthread_should_stop()) {
		spin_lock(&lock); // acquire lock
		// if size of queue is zero, then wait for filling
		if(GetSizeList(queue) == 0) {
			spin_unlock(&lock); // release lock for filling queue
			wait_for_completion(&empty); // wait for filling queue
			spin_lock(&lock); // acquire lock after filling queue
		}
...
static int consumer_func3(void *data) {
	...
	// define queu with data from parameter of function
	list_t *queue = (list_t *)data;
	int num;
...
static int __init kthread_task3_init(void) {
	init_completion(&empty); // initialize completion variable, empty
	spin_lock_init(&lock); // initalize spin lock variable, lock
